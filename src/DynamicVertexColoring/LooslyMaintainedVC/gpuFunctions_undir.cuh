	...
	for (int index = blockIdx.x * blockDim.x + threadIdx.x; index < totalChangeEdges_Del; index += blockDim.x * gridDim.x)
	{
		...
		for (int j = AdjListTracker_device[targeted_node]; j < AdjListTracker_device[targeted_node + 1]; j++) {
			if (AdjListFull_device[j].flag != -1 && vertexcolor[AdjListFull_device[j].col] == vertexcolor[other_node])
			{
				...
				//return; //prev we were returning, that was wrong
				flag = 1;
			}
		}
...
/// sc_mask_startingBlock sends the 1st element to start from while checking available color.
/// </summary>
...
/// <param name="change"></param>
/// <returns></returns>
__global__ void findEligibleNeighbors(int* affectedNodeList, ColWt* AdjListFull_device, int* AdjListTracker_device, int* affected_marked, int* previosVertexcolor, int* vertexcolor, int* counter, int* change, int iteration, int max_iteration) {
	for (int index = blockIdx.x * blockDim.x + threadIdx.x; index < *counter; index += blockDim.x * gridDim.x)
	{
		...
		for (int j = AdjListTracker_device[node]; j < AdjListTracker_device[node + 1]; j++)
		{
			...
			//else if (AdjListFull_device[j].flag > -1 && previosVertexcolor[node] < vertexcolor[AdjListFull_device[j].col] && iteration < max_iteration)
			//{
