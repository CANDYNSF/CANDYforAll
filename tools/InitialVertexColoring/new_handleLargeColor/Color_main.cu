	...
	*counter = cuCompactor::compact<int, int>(affected_marked, affectedNodeList, nodes, predicate(), THREADS_PER_BLOCK);
	...
	while (*change > 0)
	{
		*change = 0;
		...
		findEligibleNeighbors << < numberOfBlocks, THREADS_PER_BLOCK >> > (affectedNodeList, AdjListFull_device, AdjListTracker_device, affected_marked, previosVertexcolor, vertexcolor, counter, change);
